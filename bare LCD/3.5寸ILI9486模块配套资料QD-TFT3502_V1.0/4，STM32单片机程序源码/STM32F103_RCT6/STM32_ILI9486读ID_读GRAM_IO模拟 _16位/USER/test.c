...
u16 ColorTab2[10]={RED,GREEN,BLUE,WHITE,BLACK,YELLOW,GBLUE,BRRED,GRAY,BROWN};
u16 ColorTab3[10]={LBBLUE,LGRAYBLUE,LGRAY,LIGHTGREEN,DARKBLUE,MAGENTA,CYAN,BRRED,GRAY,BROWN};
void ReadRAM_Test(void)
{
	u8 sizex=50;
	u8 sizey=30;
	u16 Color_Read=0;
	u16 i=0;
	LCD_Clear(MAGENTA);
	Show_Str(10,10,BLUE,YELLOW,"Write",16,1);

	Show_Str(100,10,BLUE,YELLOW,"ReadRGB",16,1);
	
	Show_Str(180,10,BLUE,YELLOW,"Result",16,1);
	for(i=0;i<10;i++)
	{
	LCD_Fill(10,40+i*40,10+sizex,40+i*40+sizey,ColorTab2[i]);			
	Color_Read=LCD_ReadPoint(10+5,40+i*40+5);	
	LCD_Fill(100,40+i*40,100+sizex,40+i*40+sizey,Color_Read);
	if(Color_Read==ColorTab2[i])
	Show_Str(180,45+i*40,BLUE,YELLOW,"OK",16,1);	
	else
	Show_Str(180,45+i*40,RED,YELLOW,"ERROR",16,1);	
	//LCD_Fill(180,40+i*40,180+sizex,40+i*40+sizey,LCD_BGR2RGB(Color_Read));
	}
	delay_ms(1500);
	LCD_Clear(GREEN);
	...
	for(i=0;i<10;i++)
	{
	LCD_Fill(10,40+i*40,10+sizex,40+i*40+sizey,ColorTab3[i]);			
	Color_Read=LCD_ReadPoint(10+5,40+i*40+5);	
	LCD_Fill(100,40+i*40,100+sizex,40+i*40+sizey,Color_Read);
	if(Color_Read==ColorTab3[i])
	Show_Str(180,45+i*40,BLUE,YELLOW,"OK",16,1);	
	else
	Show_Str(180,45+i*40,RED,YELLOW,"ERROR",16,1);	
	//LCD_Fill(180,40+i*40,180+sizex,40+i*40+sizey,LCD_BGR2RGB(Color_Read));
...
{	
	...
	for(i=0;i<0xFFFE;i++)
	{
		LCD_Fill(10,40,40,70,i);
		Color_Read=LCD_ReadPoint(15,45);
		LCD_Fill(80,40,110,70,Color_Read);		
		if(Color_Read!=i)
		{
			Show_Str(180,45,RED,YELLOW,"ERROR",16,1);
			return 0;
		}
		else
		{
			Show_Str(180,45,RED,YELLOW,"OK",16,1);
		}
		POINT_COLOR=RED;
		LCD_ShowNum(220,45,i,5,16);

	}	
	Show_Str(180,150,BLUE,YELLOW,"GRAM Read test PASS!",16,1);
	delay_ms(1200);
	return 1;	
}
u8 Read_ID_Test(void)
{	
	u8 lcd_id[15];			//存放LCD ID字符串	
	u16 LCD_ID=0;
	POINT_COLOR=RED; 
	LCD_Clear(WHITE);
	LCD_ID=LCD_ReadID(0xD3);//ID读取测试返回0x9488代表正常
	sprintf((char*)lcd_id,"LCD ID:0x%04X",LCD_ID);//将LCD ID打印到lcd_id数组。	
	Show_Str(10,20,BLUE,YELLOW,lcd_id,16,1);	
	delay_ms(1200);
}	
