...
void Draw_Button(uint16_t xstart,uint16_t ystart,uint16_t xend,uint16_t yend)
{
	...
	TFT_Fill(xstart+2, ystart+2, xend-2, yend-2,LGRAY);//填充中间颜色
}
...
{
	POINT_COLOR=BLACK;
	TFT_DrawLine(xstart, ystart, xend, ystart);//画顶部横线1
	TFT_DrawLine(xstart, ystart, xstart, yend);//画左边竖线1
	POINT_COLOR=DARKGRAY;
	TFT_DrawLine(xstart+1, ystart+1, xend-1, ystart+1);//画顶部横线2
	TFT_DrawLine(xstart+1, ystart+1, xstart+1, yend-1);//画左边竖线2
	POINT_COLOR=LGRAY;
	TFT_DrawLine(xstart+1, yend-1, xend-1, yend-1);//画底部横线1
	TFT_DrawLine(xend-1, ystart+1, xend-1, yend-1);//画右边竖线1	
	POINT_COLOR=WHITE;
	TFT_DrawLine(xstart, yend, xend, yend);//画底部横线2
	TFT_DrawLine(xend, ystart, xend, yend);//画右边竖线2
}
...
void EscButton(uint16_t xstart,uint16_t ystart,uint8_t xend,uint16_t yend)
{
	...
	TFT_DrawLine(xstart+1, ystart+1, xstart+1, yend-1);//画左边竖线2
	POINT_COLOR=BLACK;
	TFT_DrawLine(xstart, yend, xend, yend);//画底部横线1
	TFT_DrawLine(xend, ystart, xend, yend);//画右边竖线1
	POINT_COLOR=DARKGRAY;
	TFT_DrawLine(xstart+1, yend-1, xend-1, yend-1);//画底部横线2
	TFT_DrawLine(xend-1, ystart+1, xend-1, yend-1);//画右边竖线2
}
...
void Draw_TextBox(uint16_t xstart, uint16_t ystart, uint16_t xend, uint16_t yend)
{
	POINT_COLOR=DARKGRAY;
	TFT_DrawLine(xstart, ystart, xend, ystart);//画顶部横线1
	TFT_DrawLine(xstart, ystart+1, xstart, yend);//画左面竖线1
	POINT_COLOR=BLACK;
	TFT_DrawLine(xstart+1, ystart+1, xend-1, ystart+1);//画顶部横线2
	TFT_DrawLine(xstart+1, ystart+2, xstart+1, yend-1);//画左面竖线2
	POINT_COLOR=WHITE;
	TFT_DrawLine(xstart, yend, xend, yend);//画底部横线1
	TFT_DrawLine(xend, ystart, xend, yend);//画右面竖线1
	POINT_COLOR=LGRAY;
	TFT_DrawLine(xstart+1, yend-1, xend-1, yend-1);//画底部横线2
	TFT_DrawLine(xend-1, ystart+1, xend-1, yend-1);//画右面竖线2
...
void Draw_Window(uint16_t xstart,uint16_t ystart,uint8_t xend,uint16_t yend,uint8_t* caption)
{
	...
	TFT_Fill(xstart+3, ystart+3, xend-3, ystart+25, DARKBLUE);	 // 显示标题栏
...
{
 	POINT_COLOR = DARKGRAY;
	TFT_DrawLine(xstart, ystart, xend, ystart);	  // 上边框
	TFT_DrawLine(xstart, yend, xend, yend);	// 下边框
	TFT_DrawLine(xstart, ystart, xstart, yend);	// 左边框
	TFT_DrawLine(xend, ystart, xend, yend);	   // 右边框

	POINT_COLOR = WHITE;
	TFT_DrawLine(xstart+1, ystart+1, xend-1, ystart+1); // 上边框灯光
	TFT_DrawLine(xstart, yend+1, xend+1, yend+1);  // 下边框灯光
	TFT_DrawLine(xstart+1, ystart+1, xstart+1, yend-1);	  // 左边框灯光
	TFT_DrawLine(xend+1, ystart, xend+1, yend+1);		 // 右边框灯光

	POINT_COLOR = BLACK;
	BACK_COLOR = LGRAY;
	TFT_ShowString(xstart+5, ystart-6, FrameName);	 // 显示Frame名称
}
...
void POINT_Demo(void)
{
	...
	for(i=0;i<2;i++)
	{
		TFT_Draw9Point(x, y-r, WHITE); //画第一个点
		delay_ms(m);
		TFT_Draw9Point(x+t, y-t, WHITE); //画第二个点
		delay_ms(m);
		TFT_Draw9Point(x, y-r, BLACK); //删除第一个点
		delay_ms(m);
		TFT_Draw9Point(x+r, y, WHITE); //画第三个点
		delay_ms(m);
		TFT_Draw9Point(x+t, y-t, BLACK);  //删除第二个点
		delay_ms(m);
		TFT_Draw9Point(x+t, y+t, WHITE);  //画第四个点
		delay_ms(m);
		TFT_Draw9Point(x+r, y, BLACK); //删除第三个点
		delay_ms(m);
		TFT_Draw9Point(x, y+r, WHITE);  //画第五个点
		delay_ms(m);
		TFT_Draw9Point(x+t, y+t, BLACK);  //删除第四个点
		delay_ms(m);
		TFT_Draw9Point(x-t, y+t, WHITE);  //画第六个点
		delay_ms(m);
		TFT_Draw9Point(x, y+r, BLACK);  //删除第五个点
		delay_ms(m);
		TFT_Draw9Point(x-r, y, WHITE);  //画第七个点
		delay_ms(m);
		TFT_Draw9Point(x-t, y+t, BLACK);  //删除第六个点
		delay_ms(m);
		TFT_Draw9Point(x-t, y-t, WHITE);  //画第八个点
		delay_ms(m);
		TFT_Draw9Point(x-r, y, BLACK);  //删除第七个点
		delay_ms(m);
		TFT_Draw9Point(x-t, y-t, BLACK);  //删除第八个点
		delay_ms(m);
	}
}
...
void ProgresBar_Demo(void)
{
	...
	TFT_Clear(BLUE);	   // 整屏显示红色
	Draw_Button(100, 210, 230, 310);   // 显示主体窗口
	TFT_Fill(103, 213, 227, 235, DARKBLUE);	 // 显示标题栏 
	Draw_Button(105, 280, 155, 305);  // 显示第一个按钮
	Draw_Button(175, 280, 225, 305);  // 显示第二个按钮
	POINT_COLOR=BLACK;	
	BACK_COLOR=LGRAY;
	TFT_ShowString( 114, 284, "确定"); // 按钮上写字
	TFT_ShowString( 184, 284, "退出");
...
void ComboDemo(void)
{
	...
	//写列表中的内容
	TFT_Fill(51, 75, 199, 97, DARKBLUE); //第一个默认为选中状态
	POINT_COLOR=WHITE;
	BACK_COLOR=DARKBLUE;
	...
	delay_ms(500);
	//选中第二个
	TFT_Fill(51, 75, 199, 97, WHITE);	   //先取消第一个
	POINT_COLOR=BLACK;
	BACK_COLOR=WHITE;
	TFT_ShowString(53, 79, "NXP ICP Bridge");

	TFT_Fill(51, 98, 199, 120, DARKBLUE);  //选中第二个
	POINT_COLOR=WHITE;
	BACK_COLOR=DARKBLUE;
	TFT_ShowString(53, 101, "NXP PP Bridge");
	delay_ms(500);
	...
	delay_ms(500);
	//选中第三个
	TFT_Fill(51, 98, 199, 120, WHITE);//先取消选中的第二个
	POINT_COLOR=BLACK;
	BACK_COLOR=WHITE;
	TFT_ShowString(53, 101, "NXP PP Bridge");

	TFT_Fill(51, 121, 199, 142, DARKBLUE);//选中第三个
	POINT_COLOR=WHITE;
	BACK_COLOR=DARKBLUE;
	TFT_ShowString(53, 124, "None ISP");
	delay_ms(500);
...
{		
	...
	POINT_COLOR = BLACK;
	BACK_COLOR = LGRAY;
	TFT_ShowString(187,67,"搜索");		 // 按钮上写字
	
	POINT_COLOR = BLUE;
	BACK_COLOR = WHITE;
	TFT_ShowString(30,38,"新闻");
	TFT_DrawLine(30,55,62,55);
	TFT_ShowString(110,38,"图片");
	TFT_DrawLine(110,55,142,55);
	POINT_COLOR = BLACK;
	TFT_ShowString(70,38,"标签");
	delay_ms(500);
	delay_ms(500);
	delay_ms(500);
	...
	POINT_COLOR = BLACK;
	BACK_COLOR = LGRAY;
	TFT_ShowString(188,68,"搜索");		 
	delay_ms(500);

	EscButton(180, 60, 230, 90);			 // 放开“搜索”按钮
	POINT_COLOR = BLACK;
	BACK_COLOR = LGRAY;
	TFT_ShowString(187,67,"搜索");
	POINT_COLOR = BLACK;
	BACK_COLOR = WHITE;		 
	TFT_ShowString(12,100,"ARM Cortex-M0处理器是现有的最小、能耗最低和能效最高的ARM处理器。该处理器硅面积极小、能耗极低并且所需的代码量极少，这使得开发人员能够以8位的设备实现32位设备的性能，从而省略16位设备的研发步骤。Cortex-M0 处理器超低的门数也使得它可以部署在模拟和混合信号设备中。");
	delay_ms(500);
	delay_ms(500);
	delay_ms(500);
	delay_ms(500);
